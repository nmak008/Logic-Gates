...
def FA(x,y,cin):
    ...
    sum is the low bit of x+y+cin and carry _out is
    the high bit of x+y+carry_in.'''
    S = gor(gand(gnot(x), XOR(y, cin)), gand(x, gnot(XOR(cin, y))))
    cout = gor(gand(y, cin), gand(x, gor(gand(gnot(y), cin), gand(y, gnot(cin)))))
    return (cout, S)
...
def fourBitAdd(xxxx,yyyy):
    ...
    and c is the carry.'''
    (a, z) = FA(xxxx[3], yyyy[3], 0)
    (b, zz) = FA(xxxx[2], yyyy[2], a)
    (c, zzz) = FA(xxxx[1], yyyy[1], b)
    (d, zzzz) = FA(xxxx[0], yyyy[0], c)
    return (d, (zzzz, zzz, zz, z))
